---
name: New tool
about: Suggest a tool for the GitDeck registry
labels: tool
---

**Tool name and website:**

**URL transformation** (how a GitHub repository is passed to it):

```
https://example.com/{owner}/{repo}
```

**Verification** — paste the result of requesting the transformed URL for a known repository:

```
curl -sIL -A "Mozilla/5.0" "https://example.com/facebook/react"
```

**Category:** editor / understand / visualise / AI context / insights / supply chain / search

**Does it require an account?** (Tools behind a sign-in wall are generally not accepted.)
